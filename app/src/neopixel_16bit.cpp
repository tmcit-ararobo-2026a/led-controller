...
Neopixel16bit::Neopixel16bit(TIM_HandleTypeDef* htim, uint16_t channel, int num_pixels)
...
Neopixel16bit::~Neopixel16bit()
{
    delete[] pixels;
    delete[] pwm_data;
}

bool Neopixel16bit::show()
{
    if (data_sent) {
...
void Neopixel16bit::fill(uint8_t r, uint8_t g, uint8_t b)
{
    for (uint8_t i = 0; i < num_pixels; i++) {
        pixels[i][0] = g;
        pixels[i][1] = r;
...
void Neopixel16bit::set_pixel_color(
...
void Neopixel16bit::pulse_sent_callback(TIM_HandleTypeDef* htim)
{
...
void Neopixel16bit::flash_sky_tree(
    uint8_t min_pixel,
...
void Neopixel16bit::gradually_shine(
    uint8_t min_pixel, uint8_t max_pixel, uint8_t r, uint8_t g, uint8_t b
)
{
    if (shine_flag) {
...
void Neopixel16bit::gradually_dark(uint8_t min_pixel, uint8_t max_pixel)
{
    if (dark_flag) {
        if (0 <= led_r - 10) {
            led_r = led_r - 10;
